...
- To have the cut songs back you'll need to extract the STREAMS audio files with saat 1.10 by alci (the one that comes with a readme) from your downgraded steam/rockstar launcher version or from the disc version that has the cut songs:
  ...
  - You need also saaf by nick7 build 239 to use it to open each .osw file, it'll create an `.osw.idx` file.
